...
@Component({
    ...
    template : `
                <h2>User Review:</h2>
                <textarea class="textfield" placeholder="Write your Review" [value]="review_input" (input) = getInputValue($event)></textarea>
                <br/><br/>
                <h3>Output:</h3>
...
export class ReviewComponent {
    @Output() textArea = new EventEmitter<string>()
    // sample input
    review_input = 
...
At https://wallethub.com <b>bolded text</b>`;
    ...
    ngOnInit() {
        this.review_content = `<pre>${this.review_input}</pre>`;
    }

    getInputValue(event) {
        this.review_input = event.target.value;
        this.review_content = `<pre>${this.review_input}</pre>`;
    }
